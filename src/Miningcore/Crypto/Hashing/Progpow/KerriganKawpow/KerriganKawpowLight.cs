...
public class KerriganKawpowLight : IProgpowLight
{
    ...
    public string AlgoName { get; } = "KerriganKawPoW";
    ...
    {
        var epoch = block / ProgpowConstants.EpochLength;
        Cache result;
        ...
        lock(cacheLock)
        {
            ...
            if(!caches.TryGetValue(epoch, out result))
            {
                ...
                while(caches.Count >= numCaches)
                {
                    ...
                    logger.Info(() => $"Evicting Kerrigan KawPoW cache for epoch {epochToEvict} in favour of epoch {epoch}");
                    toEvict.Dispose();
                    caches.Remove(key);
                }
                ...
                {
                    logger.Debug(() => $"Using pre-generated Kerrigan KawPoW cache for epoch {epoch}");
                ...
                {
                    logger.Info(() => $"No pre-generated cache available, creating new Kerrigan KawPoW cache for epoch {epoch}");
            ...
            {
                logger.Info(() => $"Pre-generating Kerrigan KawPoW cache for epoch {epoch + 1}");
